...
This is the i3 window manager directory within a dotfiles repository. It contains the i3blocks status bar configuration and various utility scripts for window management and system control. The i3 config itself is not tracked here: Home Manager in the `Bad3r/nixos` flake generates `~/.config/i3/config` from `modules/apps/i3wm/`, along with `scripts/blur-lock`, `scripts/i3lock-stylix` and `scripts/toggle_logseq.sh`.
...
### File Structure
- **config** - Generated by Home Manager, not tracked (`.gitignore` lists it). `config..hm.bk` and `config.odd.bk` are pre-migration copies
...
Example from toggle_obsidian.sh:
```bash
...
1. Edit `modules/apps/i3wm/keybindings.nix` in the `Bad3r/nixos` flake
2. Rebuild so Home Manager regenerates `~/.config/i3/config`
3. Test with: `i3 -C -c ~/.config/i3/config`
4. Apply with: `i3-msg reload`

### Status Bar Customization
The configuration uses i3status-rs instead of i3blocks. Home Manager generates `~/.config/i3status-rust/` from the flake. To modify:
1. Edit `modules/apps/i3status-rust.nix` or `modules/apps/i3wm/status.nix` in the flake
2. Rebuild
